    ...
    public class PatchController
    {
        ...
        private IEnumerator InitPackage()
        {
            ...
            switch (AssetMgr.Instance.playMode)
            {
                ...
                {
                        Log("WebGL 运行模式 (匹配版本特有 API)");
                        string hostURL = GetHostServerURL();
                        var remoteServices = new RemoteServices(hostURL, hostURL);

                        // 1. 创建内置资源文件系统参数
                        var webServerParams = FileSystemParameters.CreateDefaultWebServerFileSystemParameters();

                        // 2. 创建远端资源文件系统参数 (通常这个命名也会是对称的)
                        // 如果 CreateDefaultWebRemoteFileSystemParameters 报错，请尝试使用下面这一行
                        var webRemoteParams = FileSystemParameters.CreateDefaultWebRemoteFileSystemParameters(remoteServices);

                        initParams = new WebPlayModeParameters
                        {
                            WebServerFileSystemParameters = webServerParams,
                            WebRemoteFileSystemParameters = webRemoteParams
                        };
                        break;
                }
            }
